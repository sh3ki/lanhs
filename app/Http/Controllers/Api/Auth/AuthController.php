...
class AuthController extends Controller
{
     ...
     * @throws Exception
     */
    /** changed to the new logout below
  ...
  //      $user = Auth::user();
   //     $user->currentAccessToken()->delete();
    ...
    public function logout(): JsonResponse
    {
        ...
        // Check if it's an actual PersonalAccessToken (not TransientToken)
        if ($user) {
        ...
        // For session-based authentication (if session exists)
    ...
    public function register(RegisterRequest $request): JsonResponse
    {
        ...
        // Use provided role_id or default to app_default_role
        $user->role_id = $request->get('role_id') ?? Setting::getDecoded('app_default_role');
        $user->save();
    ...
    public function check(Request $request): JsonResponse
    {
        ...
        if ($access) {
            /** @var User $user */
            $user = Auth::user();
            $dashboardAccess = $user->checkEffectiveDashboardAccess();
            if ($request->get('controller')) {
                $access = $user->checkEffectivePermission($request->get('controller'));
            }
        }
