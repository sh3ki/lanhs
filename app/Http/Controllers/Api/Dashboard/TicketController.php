...
use App\Notifications\Ticket\TicketAssignedToTechnician;
use Auth;
use Carbon\Carbon;
use Exception;
...
class TicketController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        if ($user->role_id !== 1) {
            $items = Ticket::with(['ticketReplies', 'status', 'priority', 'department', 'labels', 'user', 'agent', 'closedBy'])
                ->filter($request->all())
    ...
    public function show(Ticket $ticket): JsonResponse
    {
        ...
        $ticket->load(['status', 'priority', 'department', 'labels', 'user', 'agent', 'closedBy', 'ticketReplies.user', 'ticketReplies.ticketAttachments']);
    ...
    {
        // Only non-admin dashboard users (technicians) appear in the assign dropdown
        $roles = UserRole::where('dashboard_access', true)->where('id', '!=', 1)->pluck('id');
    ...
    public function ticketQuickActions(Ticket $ticket, Request $request): JsonResponse
    {
        ...
        if ($action === 'agent') {
            ...
            if ($value) {
                $assignedAgent = User::find($value);
                if ($assignedAgent) {
                    $assignedAgent->notify(new TicketAssignedToTechnician($ticket));
                }
            }
    ...
    // Add these methods to your App\Http\Controllers\Api\Dashboard\TicketController

    public function getNewTicketCount(Request $request)
    {
        try {
            // Get user's last check time (you might want to store this in user preferences)
            $lastCheck = $request->user()->last_ticket_check ?? now()->subHours(24);
            
            // Count new tickets since last check - adjust the query based on your needs
            $newTicketCount = \App\Models\Ticket::where('created_at', '>', $lastCheck)
                ->where('status_id', 1) // Assuming status_id 1 means 'new' - adjust as needed
                ->count();
    ...
    public function markTicketsAsRead(Request $request)
    {
        try {
            // Update user's last ticket check time
            $request->user()->update([
                'last_ticket_check' => now()
            ]);
            
            return response()->json(['message' => 'Tickets marked as read']);
