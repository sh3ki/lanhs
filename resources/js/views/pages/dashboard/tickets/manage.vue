            ...
            <div :style="{height: 'calc(100vh - 70px)'}" class="flex-auto min-w-0">
                <div class="w-full h:auto sm:h-14 border-b sm:px-3">
                    <div class="sm:flex sm:justify-between">
                        <div class="flex">
                            <button class="btn p-4 rounded-none" type="button" @click="$router.push('/dashboard/tickets').catch(() => {})">
                        ...
                        <template v-if="ticket.ticketReplies.length > 0">
                            <div class="block">
                                <template v-for="(ticketReply, index) in ticket.ticketReplies">
                                    <div v-if="ticketReply.user" :class="{'border-t' : index > 0}" class="flex p-6">
                                        <img
                                            :alt="$t('Avatar')"
    ...
    methods: {
        getTicket() {
            ...
            axios.get('api/dashboard/tickets/' + self.$route.params.uuid).then(function (response) {
                ...
                self.$router.push('/dashboard/tickets').catch(() => {});
            });
        },
        getCannedReplies() {
            const self = this;
