    ...
    methods: {
        getTicket() {
            ...
            axios.get('api/tickets/' + self.$route.params.uuid).then(function (response) {
                ...
                // Mark notifications for this ticket as read when conversation is opened
                axios.post('api/dashboard/notifications/mark-read', { ticket_uuid: self.$route.params.uuid }).then(function () {
                    // Immediately remove this UUID from Vuex so the dot is gone when user goes back to list
                    const updated = (self.$store.state.unreadTicketUuids || []).filter(u => u !== self.$route.params.uuid);
                    self.$store.commit('setUnreadTicketUuids', updated);
                }).catch(function () {});
            }).catch(function () {
        ...
        uploadFile(e) {
            ...
            axios.post(
                ...
                self.ticketReply.attachments.push(attachment);
            }).catch(function () {
                self.loading.file = false;
                self.uploadingFileProgress = 0;
                self.$refs.fileInput.value = null;
