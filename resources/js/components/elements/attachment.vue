<template>
    <div class="flex items-center border w-full h-12 py-3 px-2 mb-1 mr-1">
        <div class="mr-2 flex-shrink-0">
            <img v-if="previewSrc" :src="previewSrc" :alt="details.name" class="h-8 w-8 object-cover rounded" @error="previewError = true"/>
...
export default {
    ...
    computed: {
        isImage() {
            return this.details.mime && this.details.mime.startsWith('image/');
        },
        previewSrc() {
            if (this.previewError) return null;
            // Local blob URL from fresh upload
            if (this.details.previewUrl) return this.details.previewUrl;
            // Backend preview URL (public URL for public disk, inline download for private)
            if (this.details.preview) {
                if (this.details.url !== null && this.details.url !== undefined) {
                    return this.details.preview;
                }
                return this.details.preview + '&token=' + localStorage.getItem('token');
            }
            return null;
        }
    },
